...
#   2050 CNRM-CM5 #
#####################
occurdat<-list.files("D:\\IPPC 5 Data\\2050 (average for 2041-2060)\\CNRM-CM5\\",pattern=".tif$",full=T)
months <- seq(0,96,12)
for (i in months){
...
cnprtntx701 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, cnprtntx701)
...
for (i in 1:12)
{
  col1 <- paste("cn26tmean50",i, sep="")
  x10[,col1] <- NA
}
for (i in 1:12)
{
  col2 <- paste("cn45tmean50",i, sep="")
  x10[,col2] <- NA
}
for (i in 1:12)
{
  col4 <- paste("cn85tmean50",i, sep="")
  x10[,col4] <- NA
}
names(x10)
...
occurdat<-list.files("D:\\IPPC 5 Data\\2070 (average for 2061-2080)\\CNRM-CM5\\",pattern=".tif$",full=T)
months <- seq(0,96,12)
for (i in months){
  occurdat[(1+i):(12+i)] <- c(occurdat[1+i],occurdat[(5+i):(12+i)],occurdat[(2+i):(4+i)])
}
...
for (i in 1:12)
{
  col1 <- paste("cn26tmean70",i, sep="")
  x10[,col1] <- NA
}
for (i in 1:12)
{
  col2 <- paste("cn45tmean70",i, sep="")
  x10[,col2] <- NA
}
for (i in 1:12)
{
  col4 <- paste("cn85tmean70",i, sep="")
  x10[,col4] <- NA
}
names(x10)
x11 <- x10[, c(1:36, 109:120, 37:72, 121:132,  73:108,  133:144)]

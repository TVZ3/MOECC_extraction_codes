...
gsprtntx501 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, gsprtntx501)
...
names(x11)
for (i in c(3813:3824, 3861:3872, 3909:3920, 3957:3968)){
  for (j in 1:104){
...
x11<-read.csv("TVZ_Algal.csv")
names(x11)
for (i in c(4005:4016, 4053:4064, 4101:4112, 4149:4160)){
  for (j in 1:104){
    x11[j,i+24]<-(x11[j,i]+x11[j,i+12])/2
  }
}

...
#####################
#   2050 CESM1-CAM5-1-FV2 #
#####################
occurdat<-list.files("D:\\IPPC 5 Data\\2050 (average for 2041-2060)\\CESM1-CAM5-1-FV2\\",pattern=".tif$",full=T)
months <- seq(0,24,12)
for (i in months){
...
ceprtntx701 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, ceprtntx701)
...
x10<-data.frame(x6,x2)
names(x10)

for (i in 1:12)
{
  col2 <- paste("ce45tmean50",i, sep="")
  x10[,col2] <- NA
}

names(x10)
x11 <- x10[, c(1:36, 37:48)]
names(x11)
for (i in c(13:24)){
  for (j in 1:45){
...
#####################
#   2070 CESM1-CAM5-1-FV2  #
#####################

occurdat<-list.files("D:\\IPPC 5 Data\\2070 (average for 2061-2080)\\CESM1-CAM5-1-FV2\\",pattern=".tif$",full=T)
months <- seq(0,24,12)
for (i in months){
  occurdat[(1+i):(12+i)] <- c(occurdat[1+i],occurdat[(5+i):(12+i)],occurdat[(2+i):(4+i)])
}

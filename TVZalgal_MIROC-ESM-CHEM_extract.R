...
miprtntx501 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, miprtntx501)
...
names(x11)
for (i in c(1125:1136, 1173:1184, 1221:1232, 1269:1280)){
  for (j in 1:104){
...
x11<-read.csv("TVZ_Algal.csv")
names(x11)
for (i in c(1317:1328, 1365:1376, 1413:1424, 1461:1472)){
  for (j in 1:104){
    x11[j,i+24]<-(x11[j,i]+x11[j,i+12])/2
  }
}

...
#   2050 ACCESS1-0 #
#####################
occurdat<-list.files("D:\\IPPC 5 Data\\2050 (average for 2041-2060)\\ACCESS1-0\\",pattern=".tif$",full=T)
months <- seq(0,60,12)
for (i in months){
...
acprtntx701 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, acprtntx701)
...
for (i in 1:12)
{
  col2 <- paste("ac45tmean50",i, sep="")
  x10[,col2] <- NA
}
for (i in 1:12)
{
  col4 <- paste("ac85tmean50",i, sep="")
  x10[,col4] <- NA
}
names(x10)
...
occurdat<-list.files("D:\\IPPC 5 Data\\2070 (average for 2061-2080)\\ACCESS1-0\\",pattern=".tif$",full=T)
months <- seq(0,60,12)
for (i in months){
  occurdat[(1+i):(12+i)] <- c(occurdat[1+i],occurdat[(5+i):(12+i)],occurdat[(2+i):(4+i)])
}
...
for (i in 1:12)
{
  col2 <- paste("ac45tmean70",i, sep="")
  x10[,col2] <- NA
}
for (i in 1:12)
{
  col4 <- paste("ac85tmean70",i, sep="")
  x10[,col4] <- NA
}
names(x10)
x11 <- x10[, c(1:36, 73:84, 37:72, 85:96)]

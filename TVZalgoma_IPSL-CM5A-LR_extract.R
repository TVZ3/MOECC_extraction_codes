...
occurdat<-list.files("D:\\IPPC 5 Data\\2050 (average for 2041-2060)\\IPSL-CM5A-LR\\",pattern=".tif$",full=T)
months <- seq(0,132,12)
for (i in months){
...
ipprtntx701 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, ipprtntx701)
...
for (i in 1:12)
{
  col1 <- paste("ip26tmean50",i, sep="")
  x10[,col1] <- NA
}
for (i in 1:12)
{
  col2 <- paste("ip45tmean50",i, sep="")
  x10[,col2] <- NA
}
for (i in 1:12)
{
  col3 <- paste("ip60tmean50",i, sep="")
  x10[,col3] <- NA
}
for (i in 1:12)
{
  col4 <- paste("ip85tmean50",i, sep="")
  x10[,col4] <- NA
}
names(x10)
...
occurdat<-list.files("D:\\IPPC 5 Data\\2070 (average for 2061-2080)\\IPSL-CM5A-LR\\",pattern=".tif$",full=T)
months <- seq(0,132,12)
for (i in months){
  occurdat[(1+i):(12+i)] <- c(occurdat[1+i],occurdat[(5+i):(12+i)],occurdat[(2+i):(4+i)])
}
...
for (i in 1:12)
{
  col1 <- paste("ip26tmean70",i, sep="")
  x10[,col1] <- NA
}
for (i in 1:12)
{
  col2 <- paste("ip45tmean70",i, sep="")
  x10[,col2] <- NA
}
for (i in 1:12)
{
  col3 <- paste("ip60tmean70",i, sep="")
  x10[,col3] <- NA
}
for (i in 1:12)
{
  col4 <- paste("ip85tmean70",i, sep="")
  x10[,col4] <- NA
}
names(x10)
x11 <- x10[, c(1:36, 145:156, 37:72, 157:168, 73:108, 169:180, 109:144, 181:192)]

...
ipprtntx501 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, ipprtntx501)
...
names(x11)
for (i in c(2277:2288, 2325:2336, 2373:2384, 2421:2432)){
  for (j in 1:104){
...
x11<-read.csv("TVZ_Algal.csv")
names(x11)
for (i in c(2469:2480, 2517:2528, 2565:2576, 2613:2624)){
  for (j in 1:104){
    x11[j,i+24]<-(x11[j,i]+x11[j,i+12])/2
  }
}

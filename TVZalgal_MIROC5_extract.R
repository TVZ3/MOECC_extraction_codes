...
mcprtntx501 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, mcprtntx501)
...
names(x11)
for (i in c(1893:1904, 1941:1952, 1989:2000, 2037:2048)){
  for (j in 1:104){
...
x11<-read.csv("TVZ_Algal.csv")
names(x11)
for (i in c(2085:2096, 2133:2144, 2181:2192, 2229:2240)){
  for (j in 1:104){
    x11[j,i+24]<-(x11[j,i]+x11[j,i+12])/2
  }
}

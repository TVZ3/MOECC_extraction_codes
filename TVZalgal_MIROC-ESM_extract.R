...
mrprtntx501 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, mrprtntx501)
...
names(x11)
for (i in c(1509:1520, 1557:1568, 1605:1616, 1653:1664)){
  for (j in 1:104){
...
x11<-read.csv("TVZ_Algal.csv")
names(x11)
for (i in c(1701:1712, 1749:1760, 1797:1808, 1845:1856)){
  for (j in 1:104){
    x11[j,i+24]<-(x11[j,i]+x11[j,i+12])/2
  }
}

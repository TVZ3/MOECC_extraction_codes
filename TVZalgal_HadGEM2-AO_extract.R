...
hdprtntx501 <- spTransform(gps, calclim@crs)
#extract data points
presence <- extract(calclim, hdprtntx501)
...
names(x11)
for (i in c(3429:3440, 3477:3488, 3525:3536, 3573:3584)){
  for (j in 1:104){
...
x11<-read.csv("TVZ_Algal.csv")
names(x11)
for (i in c(3621:3632, 3669:3680, 3717:3728, 3765:3776)){
  for (j in 1:104){
    x11[j,i+24]<-(x11[j,i]+x11[j,i+12])/2
  }
}
